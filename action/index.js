...
try {
  ...
  if (cloudProvider === 'scaleway') {
    ...
    // Now, retrieve container's endpoint, wait until it's ready with a 1 minute timeout and feed the API
    const containerEndpoint = await new Promise((resolve, reject) => {
      const interval = setInterval(async () => {
        try {
          const container = await containerApi.getContainer({ containerId: container.id });
